...
private:
    ...
    void UpdateCompositeConstantBuffer(ID3D11DeviceContext* ctx) {
        ...
        if (SUCCEEDED(ctx->Map(m_compositeConstantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &m))) {
            ...
            p->strength = m_strength; p->opacity = m_opacity; memcpy(p->tintColor, m_tintColor, sizeof(m_tintColor));
    ...
    float m_strength = 1.0f, m_opacity = 1.0f, m_blurAmount = 0.15f, m_tintColor[4] = {0}, m_noiseIntensity = 0, m_noiseScale = 100, m_noiseSpeed = 1, m_currentTime = 0;
    int m_noiseType = 0;
};
