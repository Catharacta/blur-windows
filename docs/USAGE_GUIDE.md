...
各 `BlurWindow` は、`blur_start` 時に自身の `bounds` に基づいてターゲットモニターを自動的に決定し、そのモニター専用のキャプチャセッションを固定します。
...
```
モニターA用ブラーウィンドウ → モニターAのデスクトップを固定キャプチャ
モニターB用ブラーウィンドウ → モニターBのデスクトップを固定キャプチャ
モニターC用ブラーウィンドウ → モニターCのデスクトップを固定キャプチャ
```
